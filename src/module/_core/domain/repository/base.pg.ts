...
> {
  ...
  /*
        Use for using Query Builder, example:

        let qb = this.em.createQueryBuilder(Test, "t").select(["*"]);
        const [populatedTest] = await this.setQueryBuilderOption(qb, {
            page: 1,
            pageSize: 1
        }).getResultList();
    */
  ...
  // count(filter = {}) {
  //     if (Object.keys(filter).length > 0) return this.model.countDocuments(filter).lean().exec();
  //     else return this.model.estimatedDocumentCount().lean().exec();
  // }
